 ...
 * This allows legitimate HTML tags (like <strong>, <code>, <p>) while
 * preventing script execution and event handlers.
...
function revealAnswer(wrapper, item, toggleBtn) {
  ...
  if (item.assessment_type === "matching") {
    ...
    // Explanation may contain HTML (from JSON), but since data is curated,
    // we use innerHTML. For external data, use textContent or DOMPurify.
    answerBox.innerHTML = `<div class="result ${isCorrect ? "correct" : "incorrect"}">${isCorrect ? "Correct!" : "Incorrect."}</div><p style="font-size:13px"><strong>Correct matching:</strong><br>${correctText}</p><details class="explanation-section"><summary>Reveal Explanation</summary><div>${safeRenderHTML(item.prompt.explanation)}</div></details>`;
  } else {
